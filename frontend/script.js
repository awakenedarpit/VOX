...
function exportEvaluation() {
  const payload = {
    ...
    config: { stt: 'Groq Whisper Large v3', silence_ms: SILENCE_MS, max_recording_ms: MAX_RECORDING_MS },
...
function isLikelyAssistantEcho(text) {
  ...
  return response.includes(normalized) || overlap >= 0.9;
}
...
function monitorVoice() {
  ...
  if (speechDetected && now - lastSpeechAt >= SILENCE_MS) stopRecording('silence');
...
async function transcribeBlob(blob) {
  ...
  form.append('file', blob, `vox.${extension}`);
  form.append('language', getSelectedLanguage());
  ...
  recordEvent('groq_stt_completed', { transcript_length: text.length, provider: data.provider, model: data.model });
  return text;
}
...
async function handleRecordingFinished() {
  ...
  try {
    ...
    if (isLikelyAssistantEcho(text)) {
      recordEvent('recognition_echo_ignored', { text_length: text.length });
      return;
    }
    sendQuery(text, true);
  } catch (err) {
...
async function startRecording() {
  ...
  if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
    showError('This browser does not support microphone recording. Use a current Chrome browser.');
    return;
  }
  if (!mediaStream) {
  ...
  mediaRecorder.onstop = async () => {
    ...
    if (isMicActive) setState('LISTENING', 'Listening...');
  };
  ...
  vadTimer = setInterval(monitorVoice, 100);
  recordEvent('speech_recording_started', { language: getSelectedLanguage(), mime_type: mediaRecorder.mimeType });
  setState('LISTENING', 'Listening… pause briefly when you finish speaking.');
}
...
async function toggleMic() {
  try {
    ...
    setState('IDLE', 'Allow microphone access and try again.');
  }
}

async function sendQuery(text, fromVoice = false) {
  const cleaned = text.trim().replace(/\s+/g, ' ');
  if (!cleaned || cleaned === lastSubmittedTranscript) return;
  ...
  activeResponseText = '';
  recordEvent('task_created', { task_id_created: taskId, text_length: cleaned.length, recovery_task: isRecoveryTask, input_source: fromVoice ? 'groq-whisper' : 'text' });
  ...
  try {
    ...
    else speakFallbackVoice(activeResponseText, taskId);
  } catch (err) {
    if (err.name === 'AbortError') recordEvent('request_aborted', { aborted_task_id: taskId });
    else if (taskId === currentTaskId) {
      showError(`Error: ${err.message}`);
      setState(isMicActive ? 'LISTENING' : 'IDLE', 'Could not reach backend. Verify backend is running.');
    }
  }
}
...
function speakFallbackVoice(text, taskId) {
  if (!('speechSynthesis' in window)) {
    setState('IDLE', 'Rime unconfigured and browser speech is unavailable.');
    return;
  }
  stopAudio();
  ...
  utterance.onstart = () => {
    ...
    markRecoveryPlaybackStarted(taskId);
    setState('SPEAKING', 'VOX is speaking.');
  };
  utterance.onend = () => {
    if (taskId !== currentTaskId) return;
    recordEvent('task_completed', { completed_task_id: taskId });
...
function markManualTrial() {
  const open = interruptionTrials.find(t => t.recovery_success === null);
  if (!open) {
    showError('No open interruption trial. Trigger an interruption first.');
    return;
  }
  open.recovery_success = true;
  open.manually_marked = true;
  recordEvent('trial_manually_marked_success', { trial_id: open.trial_id });
  updateEvaluationSummary();
}

function simulateFirstStep() { sendQuery('Find laptops under ₹60,000'); }

function simulateSecondStep() {
  if (currentState !== 'SPEAKING' && currentState !== 'THINKING') {
    sendQuery('Find laptops under ₹60,000');
    setTimeout(() => interrupt('Simulation interruption'), 650);
    setTimeout(() => sendQuery('Actually, make it ₹50,000'), 700);
    return;
  }
  interrupt('Simulation interruption');
  setTimeout(() => sendQuery('Actually, make it ₹50,000'), 50);
}

micBtn?.addEventListener('click', toggleMic);
stopBtn?.addEventListener('click', () => interrupt('Manual interrupt button'));
simBtn1?.addEventListener('click', simulateFirstStep);
simBtn2?.addEventListener('click', simulateSecondStep);
evalBtn?.addEventListener('click', markManualTrial);
exportBtn?.addEventListener('click', exportEvaluation);
clearBtn?.addEventListener('click', () => { transcriptEl.innerHTML = ''; });
textForm?.addEventListener('submit', event => {
  event.preventDefault();
  const value = textInput.value.trim();
  if (value) { sendQuery(value); textInput.value = ''; }
});
